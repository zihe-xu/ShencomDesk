...
4. 加载、迁移或恢复应用配置。
5. 注册包含 EventBus、TaskManager 和 FileService 的 Tauri 管理状态。
...
3. FileService 停止所有文件 watcher 并清空内存缓存。
4. TaskManager 停止接收任务，取消非终态任务并发布 `task_finished(cancelled)`。
5. 执行 `PRAGMA wal_checkpoint(TRUNCATE)`。
6. 关闭 SQLx 连接池，阻止新的数据库操作。
7. 记录数据库和应用退出结果。
8. 丢弃 tracing `WorkerGuard`，刷新三个非阻塞日志 Writer。
