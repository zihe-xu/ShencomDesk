...
插件状态存放在应用数据目录的 `plugins/<plugin-id>/state.json`，插件 Manifest 与模块一同保存在同一受管目录。状态更新先写入并同步同目录临时文件，再原子替换 `state.json`，确保中断写入不会破坏最后一个有效状态；启动时会清理遗留的状态临时文件。`updatedAtUnixMs` 不小于安装时间和上次更新时间，系统时钟回退不会导致插件在下次启动时失效。损坏或不完整的插件目录会被跳过并记录脱敏诊断，不进入运行态。
